import MinecraftData, {Item as MCDataItem} from "minecraft-data"
...
function text(username: string, message: string, whisper: boolean) {
    ...
    if (message.startsWith('_urban ')) {
        ...
        ud.term(term, (error: { message: any }, entries: { definition: any }[]) => {
            if (error) {
...
function nearestEntity(type: EntityType | null): Entity | null {
    ...
    for (const id in bot.entities) {
        let entity: Entity = bot.entities[id]
        if (type && entity.type !== type) continue
        if (entity === bot.entity) continue
        let dist: number = bot.entity.position.distanceTo(entity.position)

        if (!best || (bestDistance == null || dist < bestDistance)) {
            best = entity
            bestDistance = dist
        }
    }
